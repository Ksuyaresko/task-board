...
io.on("connection", (socket) => {
  ...
  socket.on("user:status", (status) => {
    ...
    if (user) {
      ...
      const userIndex = workspaceData.users.findIndex(
        (u) => u.id === socket.id,
      );
      if (userIndex !== -1) {
  ...
  socket.on("task:update", (taskData) => {
    const user = connectedUsers.get(socket.id);
    if (user) {
      const taskIndex = workspaceData.tasks.findIndex(
        (t) => t.id === taskData.id,
      );
      if (taskIndex !== -1) {
  ...
  socket.on("message:send", (messageData) => {
    ...
    if (user) {
      ...
      io.emit("workspace:update", workspaceData);
      console.log(
        `Message sent by ${user.name}: ${messageData.text.substring(0, 20)}...`,
      );
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (user) {
      ...
      workspaceData.users = workspaceData.users.filter(
        (u) => u.id !== socket.id,
      );
